...
class ConvNet(object):
    ...
    [[conv-[bn]-relu] x conv_layers_num - pool] x pool_layers_num - [affine-[bn]] x affine_layer_num - affine - softmax
    ...
    def __init__(self, input_dim=(3, 32, 32), num_classes=10, weight_scale=1e-3, reg=0.0,
        ...
        self.pool_layers_num = 3
        self.affine_layers_num = 1
        self.conv_pool_layers_num = self.conv_layers_num * self.pool_layers_num
        self.hidden_layers_num = self.conv_pool_layers_num + self.affine_layers_num
        ...
        if self.conv_layers_num > 0:
            layer_input_depth = [
                self.num_filters for i in xrange(self.conv_pool_layers_num)]
            layer_input_depth[0] = C

        for i in xrange(1, self.conv_pool_layers_num + 1):
            self.params['W%d' % i] = np.random.randn(
                self.num_filters, layer_input_depth[i - 1],
                self.filter_size, self.filter_size) * weight_scale
        ...
        shrink_H = self.pool_param['pool_height']**self.pool_layers_num
        shrink_W = self.pool_param['pool_width']**self.pool_layers_num
        if self.conv_layers_num > 0 and self.pool_layers_num > 0:
            shrink_size = np.prod(input_dim[1:]) * \
                self.num_filters / shrink_H / shrink_W
        else:
            shrink_size = np.prod(input_dim)
        affine_dims = [shrink_size, 300, 100, 30]

        for i in xrange(1, self.affine_layers_num + 1):
            j = self.conv_pool_layers_num + i
        ...
        self.params['W%d' % (self.total_layers_num)] = np.random.randn(
            affine_dims[self.affine_layers_num], num_classes) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        for i in xrange(1, self.conv_pool_layers_num + 1):
            ...
            if self.use_batchnorm:
                ...
                o, c = conv_bn_relu_forward(
                    o, W, b, gamma, beta, self.conv_param, self.bn_params[i - 1])
            else:
                o, c = conv_relu_forward(o, W, b, self.conv_param)
            cache.append(c)
            if i % self.conv_layers_num == 0:
                o, c = max_pool_forward_fast(o, self.pool_param)
                cache.append(c)

        for i in xrange(self.conv_pool_layers_num + 1, self.hidden_layers_num + 1):
            W = self.params['W%d' % i]
            b = self.params['b%d' % i]
            if self.use_batchnorm:
                gamma = self.params['gamma%d' % i]
        ...
        o, c = affine_forward(o, self.params['W%d' % self.total_layers_num],
                              self.params['b%d' % self.total_layers_num])
        cache.append(c)
        scores = o
        ############################################################################
        #                             END OF YOUR CODE                             #
        ...
        for i in xrange(self.hidden_layers_num, self.conv_pool_layers_num, -1):
            c = cache.pop()
            if self.use_batchnorm:
        ...
        for i in xrange(self.conv_pool_layers_num, 0, -1):
            if i % self.conv_layers_num == 0:
                c = cache.pop()
                dout = max_pool_backward_fast(dout, c)
            c = cache.pop()
            if self.use_batchnorm:
                dout, dW, db, dgamma, dbeta = conv_bn_relu_backward(dout, c)
                grads['gamma%d' % i] = dgamma
                grads['beta%d' % i] = dbeta
            else:
                dout, dW, db = conv_relu_backward(dout, c)
            grads['W%d' % i] = dW
            grads['b%d' % i] = db

...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        a_out, a_cache = affine_forward(ar_out, self.params['W2'], self.params['b2'])
        scores = a_out
        #######################################################################
        #                             END OF YOUR CODE                             #
        #######################################################################
        ...
        loss = data_loss + 0.5 * self.reg * \
            (np.sum(np.square(self.params['W1'])) +
             np.sum(np.square(self.params['W2'])))
        dax, dw2, db2 = affine_backward(dx, a_cache)
        darx, dw1, db1 = affine_relu_backward(dax, ar_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for i in range(2, self.num_layers):
            self.params['W%d' % i] = np.random.randn(hidden_dims[i - 2],
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            if self.use_batchnorm:
                o, c = affine_bnorm_relu_forward(o, self.params['W%d' % i], self.params[
                                                 'b%d' % i], self.params['gamma%d' % i], self.params['beta%d' % i], self.bn_params[i - 1])
            else:
                o, c = affine_relu_forward(
                    o, self.params['W%d' % i], self.params['b%d' % i])
            cache.append(c)
            if self.use_dropout:
                o, c = dropout_forward(o, self.dropout_param)
                cache.append(c)
        o, c = affine_forward(o, self.params['W%d' % self.num_layers],
        ...
        for i in range(1, self.num_layers + 1):
            reg_loss += np.sum(np.square(self.params['W%d' % i]))
        loss = data_loss + 0.5 * self.reg * reg_loss

        c = cache.pop()
        dx, dw, db = affine_backward(dscores, c)
        ...
        for i in range(self.num_layers - 1, 0, -1):
            ...
            c = cache.pop()
            if self.use_batchnorm:
                dx, dw, db, dgamma, dbeta = affine_bnorm_relu_backward(dx, c)
...
def affine_bnorm_relu_forward(x, w, b, gamma, beta, bn_param):
    ...
    out, bnorm_cache = batchnorm_forward(a, gamma, beta, bn_param)
...
def affine_bnorm_relu_backward(dout, cache):
    """
    Backward pass for the affine-bnorm_relu convenience layer
    """
    fc_cache, bnorm_cache, relu_cache = cache
    da = relu_backward(dout, relu_cache)
    dx, dgamma, dbeta = batchnorm_backward_alt(da, bnorm_cache)
